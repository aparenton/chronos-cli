...
def create_dep_job(server, artifact, dry_run=False):
    url = "%s/v1/scheduler/dependency" % server.url
    ...
    if dry_run:
        ...
        return result_json(r.status_code, 'Dependent job added')


def run(args):
    server = load_server(args.profile, args.config)
    artifact = load_artifact(args.profile, args.message)
    result = create_dep_job(server, artifact, args.dry_run)
    print_json(result)


def register_command(subparsers):
    parser = subparsers.add_parser('create_dep', parents=[parent_parser], help='Create a dependent job')
